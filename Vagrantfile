...
Vagrant.configure("2") do |config|
  ...
  # Configure provider-specific configuration, which is used to modify settings
  ...
  # open_memcached_app_http: Opens memcached link, either http://localhost:8080/app
  # or https://localhost:8443/app. The reason we are not opening HTTPS link because
  # it requires to add exception manually. We want to see the memcached monitoring
  # web page automatically, don't we?
